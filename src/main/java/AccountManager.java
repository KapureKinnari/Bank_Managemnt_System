...
public class AccountManager {
            ...
            if(account_number!=0){
                ...
                if(resultSet.next()){
                    ...
                    if(amount<=current_balance){
                        ...
                        if(rowsAffected > 0){
                            System.out.println("Rs" + amount + "Credited succefully");
                            connection.commit();
            ...
            if(account_number!=0){
                ...
                if(resultSet.next()){
                    ...
                    if(amount<=current_balance){
                        ...
                        int rowsAffected  = preparedStatement1.executeUpdate();
                        if(rowsAffected > 0){
                            System.out.println("Rs" + amount + "debited succefully");
                            connection.commit();
                            connection.setAutoCommit(true);
                            return;
                        }
    ...
    public void transfer_money(long sender_account_number) throws SQLException{
        ...
        try{
            ...
            if(sender_account_number!=0 && receiver_account_number!=0){
                PreparedStatement preparedStatement1 = connection.prepareStatement("SELECT balance FROM Accounts WHERE account_number = ? AND security_pin = ?");
                preparedStatement1.setDouble(1,sender_account_number);
                ...
                if(resultSet.next()){
                    ...
                    if(amount<=current_balance){
                        ...
                        int rowsAffected1 = debitPreparedStatment.executeUpdate();
                        int rowsAffected2 = creditPreparedStatement.executeUpdate();
                        if(rowsAffected1 > 0 && rowsAffected2 > 0){
                            System.out.println("Transaction Succesfull");
                            System.out.println("Rs" + amount + "Transfered Succesfull");
                            connection.commit();
                            connection.setAutoCommit(true);
                            return;
                        }else {
    ...
    public void getBalance(long account_number) throws SQLException{
        scanner.nextLine();
        System.out.println("Enter Secutiy Pin");
        String security_pin = scanner.nextLine();
        try{
            PreparedStatement preparedStatement1 = connection.prepareStatement("SELECT balance FROM Accounts WHERE account_number = ? AND security_pin = ?");
